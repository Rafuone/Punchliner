...
export default function Player() {
  ...
  const [phase, setPhase] = useState<'lobby' | 'playing' | 'reveal' | 'final'>('lobby');
  ...
  useEffect(() => {
    ...
    return () => ['lobby', 'round:go', 'round:reveal', 'game:final', 'scores:update', 'buzz:winner', 'buzz:open', 'room:closed'].forEach((e) => socket.off(e as any));
  }, []);
  ...
  useEffect(() => { const m = players.find((p) => p.id === meId.current); if (m && typeof m.charge === 'number') setCharge(m.charge); }, [players]);
    ...
    return (
      <div className="wrap">
        <div className="topbar">
          <span className="row" style={{ gap: 9 }}><span style={{ fontSize: 20 }}>🎤</span><span className="pname" style={{ fontFamily: 'var(--disp)', fontSize: 17 }}>Maître du jeu</span></span>
        ...
        {phase === 'final' && (
          <div className="center"><span className="eyebrow">Terminé</span><div className="big-num">🏁</div><h2 className="title-xl">{others.sort((a, b) => b.score - a.score)[0]?.name} gagne</h2></div>
        )}
      </div>
    );
  }
  ...
  return (
    <div className="wrap">
        ...
        <div className="center" style={{ gap: 14 }}>
          ...
          {av && (
            <div className="powerbar">
              <div style={{ flex: 1 }}>
                <div className="eyebrow" style={{ fontSize: 10 }}>{av.power.name}</div>
                <div className="gauge"><i style={{ width: `${Math.min(100, charge)}%` }} /></div>
              </div>
              <button className="btn warm" onClick={usePower} disabled={charge < 100}>{charge < 100 ? `${Math.round(charge)}%` : 'Lancer ⚡'}</button>
            </div>
          )}
      ...
      {phase === 'final' && (
        <div className="center" style={{ gap: 14 }}><span className="eyebrow">Terminé</span><div className="big-num">{myRank === 1 ? '🏆' : myRank}</div><h2 className="title-xl">{myRank === 1 ? 'Tu as gagné !' : `${myRank}ᵉ place`}</h2><p className="muted">{me?.score ?? 0} points</p></div>
      )}
    </div>
  );
}
