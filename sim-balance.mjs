...
// Collés aux 3 crans RÉELS du jeu (DIFFICULTY, server/index.js) : Mainstream 1.0 · Connaisseur 1.5 · Puriste 2.0.
// Le cran « digger » a disparu le 2026-07-11 (digger + underground fusionnés dans Puriste).
// ⚠️ pFind/pFull de 'puriste' à revoir au playtest : l'ancien puriste ne couvrait que la bande 'deep',
//    le nouveau couvre 'mid'+'deep' → il est un peu plus facile qu'avant.
const DIFFS = [
  { key: 'facile',  pFind: 0.90, pFull: 0.80, diff: 1.0 },
  { key: 'normal',  pFind: 0.72, pFull: 0.66, diff: 1.5 },
  { key: 'puriste', pFind: 0.40, pFull: 0.44, diff: 2.0 },
];
const WINDOW = 26000; // fenêtre de réponse (ms) pour modéliser le jam
// Fréquence des FAUTES de frappe/orthographe sur une réponse trouvée, et part de ces fautes qui tombent
// HORS de la tolérance normale (donc volet PERDU sans `nofault`). C'est tout ce dont vivent les pouvoirs
// « orthographe » : à ajuster au playtest réel, mais bien plus fidèle que l'ancien « volet complet garanti ».
const P_TYPO = 0.18;
const P_TYPO_LOST = 0.35;
...
function fillCharges(players) {
  ...
  sorted.forEach((p, rank) => {
    ...
    const add = 14 + fromBottom * 14; // comeback : ~14 (favorisé, ≈7 manches/charge) → ~28 (à la traîne, ≈3,5) — server/index.js
    p.charge += add;
    while (p.charge >= 100 && p.charges < 3) { p.charges += 1; p.charge -= 100; } // cap 3 (2026-07-10)
...
function playGame(avatars, rounds, prof) {
  ...
  for (let r = 0; r < rounds; r++) {
    ...
    for (const p of players) {
      ...
      let use = false;
      if (t === 'steal') use = (p.id !== leader.id && leader.score > 1800) || shielded.has(p.id);
      else if (t === 'sabotage') use = p.id !== leader.id && leader.score > 1800;
      else if (t === 'tax') use = players.some((x) => x.id !== p.id && x.score > 1200);
      else if (t === 'draft') use = p.id !== leader.id; // surfe sur un meilleur que toi
      else if (t === 'comeback') use = behind > 3600;
      else if (t === 'veteran') use = !vetActive; // (ré)active si pas déjà actif
      else if (t === 'safety') use = behind > 1800 || r < rounds - 1; // filet quand utile
      ...
      else if (t === 'tax') { players.filter((x) => x.id !== p.id && !(r <= x.vetUntil) && x.act !== 'safety' && !shielded.has(x.id)).forEach((x) => { const amt = Math.min(p.pw.amount || 1500, x.score); x.score -= amt; p.score += amt; }); }
      else if (t === 'allin') { const spent = p.charges + 1; p.score += (p.pw.per || 7200) * spent; p.charges = 0; } // +1 : la charge déjà décrémentée
      else if (t === 'sustain') { p.sustainUntil = r + ((p.pw.rounds || 2) - 1); p.sustainAmount = p.pw.amount || 4800; }
    ...
    for (const p of players) {
      let base = p.found ? (p.full ? 18000 : 6000) : 0; // 6 000/volet + prime de précision 6 000 (server/match.js)
      ...
      else if (pts > 0 && t === 'bonus') { pts += p.pw.amount; if (p.pw.refuel) p.charges = Math.min(3, p.charges + 1); } // cap 3 = serveur (index.js)
      ...
      if (vetActive) pts = Math.max(pts, p.pw.floor || 2400);
...
// GAMES surchargeable (PL_SIM_GAMES=800 node sim-balance.mjs) : itération rapide pendant un calibrage.
// ⚠️ Le bruit statistique grimpe quand GAMES baisse (~±2 pts à 1000, ~±1 pt à 4000) → toujours RE-VALIDER
// le réglage final à 4000 avant de conclure.
const GAMES = Number(process.env.PL_SIM_GAMES) || 4000, N = 5, ROUNDS = 16;
...
// En-tête GÉNÉRÉ depuis DIFFS (avant : 4 noms en dur dont « DIGGER » → colonnes décalées après le passage à 3 crans).
console.log('TIER  MOY%  ' + DIFFS.map((d) => d.key.toUpperCase().slice(0, 6).padStart(6)).join(' ') + '  SPRD  ' + 'RAPPEUR'.padEnd(13) + 'POUVOIR');
for (const r of rows) {
  const idx = r.avg / (1 / N);
